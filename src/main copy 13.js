...
window.paletteMap = {
    ...
    'h': { h: -0.50, s: 0.22, seed: 29 }, // yellow/orange wrap
    'j': { h: 0.16, s: 0.26, seed: 31 }, // yellow-green
    'k': { h: -0.70, s: 0.23, seed: 37 }, // red-magenta
    'l': { h: 0.10, s: 0.24, seed: 41 },
    ';': { h: -0.10, s: 0.24, seed: 43 },
    "'": { h: 0.00, s: 0.20, seed: 47 }
};
...
function handleKeyDown(e) {
    ...
    switch (key) {
        ...
        case 'e': // RGB shift hold
            ...
            break;
        case 'r': // DotScreen hold
            if (dotPass && dotPass.uniforms) {
                window.keyboard.holdR = true;
                dotPass.enabled = true;
                dotPass.uniforms['angle'].value = a * Math.PI;
                dotPass.uniforms['scale'].value = 1.0 - 0.8 * a;
            }
            break;
        ...
        case 'y': // Vignette hold
            if (vignettePass && vignettePass.uniforms) {
                ...
                vignettePass.uniforms['darkness'].value = 1.2 + 0.6 * a;
            }
            break;
        ...
        // Second row: palette toggles (A S D F G H J K L ; ')
        case 'a':
        case 's':
        case 'd':
        case 'f':
        ...
        case ';':
        case "'": {
            if (window.keyboard.paletteActive.has(key)) {
                window.keyboard.paletteActive.delete(key);
            } else {
                window.keyboard.paletteActive.add(key);
            }
            break;
        }
        // Third row toggles
        case 'z':
            if (bloomPass) bloomPass.enabled = !bloomPass.enabled;
            break;
        case 'x':
            if (saoPass) saoPass.enabled = !saoPass.enabled;
            break;
        case 'c':
            if (cubeTexturePass) cubeTexturePass.enabled = !cubeTexturePass.enabled;
            break;
        case 'v':
            if (afterimagePass) afterimagePass.enabled = !afterimagePass.enabled;
            break;
        case 'b':
...
function dotPulse(angleTarget = Math.PI / 3, scaleTarget = 0.5, upMs = 250, downMs = 250, delayMs = 0) {
    if (!dotPass || !dotPass.uniforms) return;
    dotPass.enabled = true;
    const start = {
        a: dotPass.uniforms['angle'].value,
        s: dotPass.uniforms['scale'].value
    };
    const peak = { a: angleTarget, s: scaleTarget };
    const back = { a: start.a, s: start.s };
    const doDown = () => new TWEEN.Tween(peak).to(back, downMs).easing(TWEEN.Easing.Quadratic.InOut).onUpdate(() => {
        dotPass.uniforms['angle'].value = peak.a;
        dotPass.uniforms['scale'].value = peak.s;
    }).start();
    setTimeout(() => {
        new TWEEN.Tween(start).to(peak, upMs).easing(TWEEN.Easing.Quadratic.Out).onUpdate(() => {
            dotPass.uniforms['angle'].value = start.a;
            dotPass.uniforms['scale'].value = start.s;
        }).onComplete(doDown).start();
    }, Math.max(0, delayMs));
}
...
function handleGamepadButtonPresses(gamepad) {
    ...
    // Button X (Square): Create dashed Hilbert spline lines instead of wireframe toggle
    if (window.gamepadState.buttonX && !window.gamepadButtonXPressed) {
        window.gamepadButtonXPressed = true;

        try {
            // Parent under selected object (editor) or current sculpture (viewer); fallback to scene
            ...
            // Remove previous set if exists
            if (window.hilbertGroup && window.hilbertGroup.parent) {
                window.hilbertGroup.parent.remove(window.hilbertGroup);
            }

            const group = new Group();
            window.hilbertGroup = group;
            parent.add(group);
            ...
            // Apply wavy offsets based on params
            ...
            // Non-standard on LineDashedMaterial, but kept per request
            clone1.material.emissive = new Color(0x88ccff);
            clone1.material.emissiveIntensity = 10.5;
            group.add(clone1);
            ...
            // Non-standard emissive props retained
            clone2.material.emissive = new Color(0x88ccff);
            clone2.material.emissiveIntensity = 10.5;
            group.add(clone2);

            console.log('🎮 Added dashed Hilbert lines');
        } catch (e) {
            console.error('Failed to create Hilbert lines:', e);
        }
    }

    ...
    switch (key) {
        ...
        case 'q': // Halftone hold
            if (halftonePass && halftonePass.uniforms) {
                ...
                if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = 1.5 + a * 2.5;
        ...
        case 'u': // Halftone hold (color)
            if (halftonePass && halftonePass.uniforms) {
                ...
                if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = sin(23) + 22.8 * a;
                if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = 15.2 + 30.8 * a;
                if (halftonePass.uniforms['blending']) halftonePass.uniforms['blending'].value = 12.0;
            }
            break;
...
function setControlsVisible(visible) {
    try {
        ...
        const cc = document.querySelector('[class^="loading-logo"]');
        if (cc) {
            if (visible) {
...
function handleGamepadTriggers(gamepad) {
    ...
    // Right Trigger (R2): fast bokeh randomization based on discrete bins + DotScreen + Halftone range
    {
        ...
        if (halftonePass && halftonePass.uniforms) {
            if (r2 > 0.85) {
                halftonePass.enabled = true;
                if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = 13.0 * r2;
                if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = 5.0 * r2;
                if (halftonePass.uniforms['greyscale']) halftonePass.uniforms['greyscale'].value = false;
                if (halftonePass.uniforms['blending']) halftonePass.uniforms['blending'].value = 5.0;
            } else {
                halftonePass.enabled = false;
            }
        }

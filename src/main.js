...
// import { Inspector } from '/src/examples/jsm/inspector/Inspector.js';
...
router.beforeEach((to, from, next) => {
	...
	if (store.state.selectedSculpture) { //fade single sculpture if selected
		...
		if (store.state.displayCanvas) {
			transitionAllSculpturesOpacity(0.04, 1000).then(() => {
				store.state.displayCanvas = false;
				nextRoute();
			});
		} else {
...
window.scene = scene;
//  camera = new THREE.PerspectiveCamera(25, window.innerWidth / window.innerHeight, 0.5, 500);
...
// // rendererNew.setPixelRatio( window.devicePixelRatio );
// // rendererNew.setSize( window.innerWidth, window.innerHeight );
// // rendererNew.setAnimationLoop( animate );
// // rendererNew.shadowMap.enabled = true;
// // rendererNew.inspector = new Inspector();
// // document.body.appendChild( rendererNew.domElement );
// //
// // controls = new OrbitControls( camera, rendererNew.domElement );
...
// // //
// //
// // const gui = rendererNew.inspector.createParameters( 'Motion Blur Settings' );
...
//
let mapControls, canvas, canvasNew, canvasOld,  canvasContainer, composer, renderPass, rgbShiftPass, filmPass, halftonePass, dotPass, afterimagePass;
// // Camera mode params
...
function setupHalftoneGUI() {
    ...
    const updateHalftoneBlending = function() {
        ...
        //
        // // Create new RawShaderMaterial with updated blending properties
        // const newMaterial = new RawShaderMaterial( {
...
function applyBackground(mode) {
    try {
        if (!rendererNew) return;
        // renderer.setClearAlpha(1);
        rendererNew.setClearAlpha(1);
        ...
        // Renderer and canvasNew
        // renderer.setClearColor(numeric, 0.5);
        rendererNew.setClearColor(numeric, 1);
        // if (renderer.domElement) renderer.domElement.style.backgroundColor = hex;
        if (rendererNew.domElement) rendererNew.domElement.style.backgroundColor = hex;
...
//     try {
//         // const el = (rendererNew && rendererNew.domElement) ? rendererNew.domElement : document.body;
//         // if (!el) return;
...
function setControlsVisible(visible) {
    try {
        ...
        // canvasNew container adjustments so no top gap remains
...
function init() {
    ...
    const floorColor = textureLoader.load( 'src/examples/textures/floors/FloorsCheckerboard_S_Diffuse.jpg' );
    floorColor.wrapS = THREE.RepeatWrapping;
    floorColor.wrapT = THREE.RepeatWrapping;
    floorColor.colorSpace = THREE.SRGBColorSpace;

    const floorNormal = textureLoader.load( 'src/examples/textures/floors/FloorsCheckerboard_S_Normal.jpg' );
    floorNormal.wrapS = THREE.RepeatWrapping;
    floorNormal.wrapT = THREE.RepeatWrapping;
    ...
    const map = new THREE.TextureLoader().load( 'src/examples/textures/uv_grid_opengl.jpg' );
    map.colorSpace = THREE.SRGBColorSpace;
    ...
    material.fragmentColor = Fn(() => {
        return vec4(sin(time), cos(time), 0.5, 1.0); // example logic
    });
    ...
    // canvasContainer = document.querySelector('.canvas-container');
    //
    // // rendererNew = new WebGPURenderer();
    // // rendererNew.autoClear = false;
    // // rendererNew.setPixelRatio( window.devicePixelRatio );
    // // rendererNew.setSize( window.innerWidth, window.innerHeight );
    // // rendererNew.shadowMap.enabled = true;
    // // // // rendererNew.inspector = new Inspector();
    // // document.body.appendChild( rendererNew.domElement );
    // // rendererNew.setSize(canvasContainer.clientWidth, canvasContainer.clientHeight);
    // // prevCanvasSize = { width: canvasContainer.clientWidth, height: canvasContainer.clientHeight };
    // // // Object.assign(store.state.canvasSize, prevCanvasSize);
    // // rendererNew.setPixelRatio(window.devicePixelRatio);
    // // rendererNew.setClearAlpha(0.5);
    // // rendererNew.setClearColor(new Color(2,3,2), 0.3);
    // // canvasContainer.appendChild(rendererNew.domElement);
    //
    //
    ...
    // //     rendererNew = new WebGLRenderer({ antialias: true });
    // //     rendererNew.setPixelRatio( window.devicePixelRatio );
    // //     rendererNew.setSize( canvasContainer.clientWidth, canvasContainer.clientHeight );
    // //     canvasContainer.appendChild( rendererNew.domElement );
    // //     // Start animation loop for WebGL
    // //     animate();
    // // } else {
    // //     console.log('Using WebGPU renderer');
    // //     rendererNew = new WebGPURenderer({ antialias: false });
    // //
    // //     try {
    // //         rendererNew.init();
    // //     } catch (e) {
    // //         console.error('rendererNew failed:', e);
    ...
    // //
	// // 	}`;
	// // materialDepth = new RawShaderMaterial({
    ...
    // //
	// // });
    // // halftonePass.material = new RawShaderMaterial( {
    // //     blending: 0.5,
    // //     // blendMode: MultiplyBlending
    // // } );
    ...
    //
	// canvasNew = document.querySelector('canvas');
    // canvasNew.setAttribute('tabindex', '0');
    // canvasNew.setAttribute('powerPreference', 'high-performance');
    // canvasNew.addEventListener('click', (event) => {
	// 	event.target.focus();
	// });
    //
	// // Add mouse interaction for bokeh focus
    // canvasNew.addEventListener('pointermove', onPointerMove);
    //
	// // canvasNew.addEventListener('mousemove', onMouseMove, false);
	...
	// };
    //
	// mapControls = new MapControls(camera, rendererNew.domElement);
...
function handleGamepadInput() {
    try {
        ...
        // Reset renderer clear color when no gamepad
            // rendererNew.setClearColor(0x000000, 1); // Green tint when moving
    }
    } catch (error) {
...
function applyShaderToScene(shaderSource) {
    ...
    if (rendererNew instanceof WebGPURenderer) {
        // For WebGPU, we need to use a compatible material
        // Since WebGPU doesn't support RawShaderMaterial directly,
        // we'll use a basic material for now and log a warning
        console.warn('RawShaderMaterial not supported in WebGPU renderer, using fallback material');
        ...
        // For WebGL, use RawShaderMaterial
        // material = new RawShaderMaterial({
        //     vertexShader: shaderSource.vertex,
        //     fragmentShader: shaderSource.fragment,
        //     uniforms: shaderSource.uniforms || {}
...
function handleGamepadTriggers(gamepad) {
    ...
    if (window.gamepadState.leftBumper && !window.gamepadButtonLeftBumperPressed) {
        ...
        if (halftonePass && halftonePass.uniforms) {
            halftonePass.enabled = true;
            if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = 20.5 + a * 2.5 * lsx;
            // setHalftoneScatter(10.5 + a);
            syncHalftoneGUI();
        }
...
function render(time) {
	...
	if (effectController.enabled) {
        ...
        // rendererNew.setRenderTarget(rtTextureColor);
		// renderer.clear();
        // rendererNew.clear();
...
//     // rendererNew.render(scene, camera);
//
//     // Continue animation loop for WebGL renderer
//     if (rendererNew instanceof WebGLRenderer) {
//         requestAnimationFrame(animate);
//     }
//
...
function piCreateMediaRecorder(isRecordingCallback, canvas)
{
    ...
    */
    ...
    var mediaRecorder = new MediaRecorder(canvasNew.captureStream(), options);
...
function transitionSculptureOpacity(sculptureId, opacity, duration = 2000) {
    ...
    console.log('transitionSculptureOpacity', transitionSculptureOpacity)

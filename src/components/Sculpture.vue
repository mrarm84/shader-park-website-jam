...
export default {
  ...
  mounted() {
    ...
    // Create meshOld copy and load third sculpture
    console.log('Creating meshOld...');
    this.sculpture.createMeshOld();

    // Load a third sculpture with different geometry
    const thirdSculptureSource = `
        let scale = input();
        sphere(scale * 0.8)
        mixGeo(box(1), sphere(scale), scale * 0.5)
    `;

    // Position all three meshes
    this.sculpture.mesh.position.x = 2;      // Right side - current
    if (this.sculpture.meshOld) {
        this.sculpture.meshOld.position.x = -2;  // Left side - old
        console.log('Meshes positioned: mesh at', this.sculpture.mesh.position.x, 'meshOld at', this.sculpture.meshOld.position.x);
    }
    if (this.sculpture.meshNew) {
        console.log('Third mesh loaded at center');
    }

    // Add all meshes to scene
    window.scene.add(this.sculpture.mesh);
    if (this.sculpture.meshOld && !window.scene.children.includes(this.sculpture.meshOld)) {
        window.scene.add(this.sculpture.meshOld);
    }
    if (this.sculpture.meshNew && !window.scene.children.includes(this.sculpture.meshNew)) {
        window.scene.add(this.sculpture.meshNew);
    }
    console.log('Added meshes to scene. meshOld exists:', !!this.sculpture.meshOld);
  ...
  watch: {
    shaderSource: function (input) {
      if (this.sculpture) {
        ...
        // Copy current mesh to meshOld before updating material
        this.sculpture.createMeshOld();

        // Reposition meshOld after creation
        if (this.sculpture.meshOld) {
            this.sculpture.meshOld.position.x = -2;
        }
        // meshNew stays at center (0, 0, -2)

        try {
          this.sculpture.refreshMaterial(processedInput);

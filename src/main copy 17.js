    ...
    switch (key) {
        ...
        case 'q': // Halftone hold
            if (halftonePass && halftonePass.uniforms) {
                ...
                if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = 10.5 + a;
            }
            break;
        case 'w': // Film hold
        ...
        case 'u': // Halftone hold (color)
            if (halftonePass && halftonePass.uniforms) {
                ...
                if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = 12.2 + 2.8 * a;
                if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = 15.2 + 0.8 * a;
            }
            break;
        ...
        case '\\': // combo glitch hold
            ...
            if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = 2.3;
            if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = 5.5;
            if (halftonePass.uniforms['greyscale']) halftonePass.uniforms['greyscale'].value = false;
            break;
        ...
        case 'a':
            if (halftonePass && halftonePass.uniforms) {
                halftonePass.enabled = true;
                if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = 2.2;
                if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = 2.5;
                if (halftonePass.uniforms['greyscale']) halftonePass.uniforms['greyscale'].value = false;
            }
            break;
        case 's':
        ...
        case 'x':
            ...
            if (e.shiftKey) {
                ...
                if (halftonePass && halftonePass.uniforms) {
                    ...
                    if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = 0.9;
                    if (halftonePass.uniforms['blending']) halftonePass.uniforms['blending'].value = 0.5;
                }
                if (dotPass && dotPass.uniforms) {
                    dotPass.enabled = true;
...
function halftonePulse(targetRadius = 2.5, targetScatter = 2.6, upMs = 300, downMs = 300, delayMs = 0) {
...
function randomizeAllParams() {
    ...
    try {
        ...
        // Halftone
        if (halftonePass && halftonePass.uniforms) {
            halftonePass.enabled = rBool(0.8);
            if (halftonePass.uniforms['greyscale']) halftonePass.uniforms['greyscale'].value = false;
            if (halftonePass.uniforms['radius']) halftonePass.uniforms['radius'].value = rIn(0.2, 3.0);
            if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = rIn(0.0, 1.0);
            if (halftonePass.uniforms['blending']) halftonePass.uniforms['blending'].value = 0.5;
        }
        // Film
        if (filmPass && filmPass.uniforms) {
    ...
    try {
        ...
        if (gamepad) {
            ...
            if (Math.abs(window.gamepadState.rightStickX) > 0.01 || Math.abs(window.gamepadState.rightStickY) > 0.01) {
                ...
                // While moving RS, map its X/Y to halftone size via setSize (R2 still controls enable)
                    if (halftonePass && typeof halftonePass.setSize === 'function' && canvasContainer) {
                        const baseW = canvasContainer.clientWidth || window.innerWidth;
                        const baseH = canvasContainer.clientHeight || window.innerHeight;
                        const fx = 0.5 + Math.min(1, Math.abs(window.gamepadState.rightStickX));
                        const fy = 0.5 + Math.min(1, Math.abs(window.gamepadState.rightStickY));
                        halftonePass.setSize(Math.max(64, baseW * fx), Math.max(64, baseH * fy));
                        if (halftonePass.uniforms && halftonePass.uniforms['greyscale']) halftonePass.uniforms['greyscale'].value = false;
                    }
                }
            }
...
function handleGamepadTriggers(gamepad) {
    ...
    // Left Trigger (L2): Film grain + RGB shift + bokeh focal (40-50)
    {
        ...
        if (l2 > 0.05) {
            ...
            // Halftone (dots) driven by L2: enable and rescale using setSize (radius/scatter moved to RS)
            if (halftonePass && typeof halftonePass.setSize === 'function' && canvasContainer) {
                halftonePass.enabled = true;
                const baseW = canvasContainer.clientWidth || window.innerWidth;
                const baseH = canvasContainer.clientHeight || window.innerHeight;
                const w = Math.max(64, baseW * (20.8 + l2 * 0.8));
                const h = Math.max(64, baseH * (20.8 + l2 * 0.8));
                try { halftonePass.setSize(w, h); } catch(e){}
                if (halftonePass.uniforms && halftonePass.uniforms['greyscale']) halftonePass.uniforms['greyscale'].value = false;
                if (halftonePass.uniforms && halftonePass.uniforms['blending']) halftonePass.uniforms['blending'].value =0.5;
            }
    ...
    // Right Trigger (R2): fast bokeh randomization based on discrete bins + DotScreen + Halftone range
    {
        ...
        if (bin >= 0 && bin !== window.prevR2Bin) {
            ...
            // if (!dotPass) dotPass.enabled = true;
        ...
        // R2: ensure halftone is enabled on hard press, but radius/scatter are controlled by Right Stick
        if (halftonePass && halftonePass.uniforms) {
            if (r2 > 0.85) {
                halftonePass.enabled = true;
                if (halftonePass.uniforms['greyscale']) halftonePass.uniforms['greyscale'].value = false;
                if (halftonePass.uniforms['blending']) halftonePass.uniforms['blending'].value = r2/100;
                if (halftonePass.uniforms['scatter']) halftonePass.uniforms['scatter'].value = r2/100;
                try { if (halftonePass.setSize) halftonePass.setSize(r2*100, r2*100); } catch(e){}
            } else {
                halftonePass.enabled = false;
            }
        }
        ...
        if (r2 > 0.95 && !window.spinR2Triggered) {
            ...
            dotPass.enabled = true;
            // angle 0..PI/2, scale 1..0.25 (smaller scale = denser dots)
            dotPass.uniforms['angle'].value = (Math.PI * 1.5) * r2;
            dotPass.uniforms['scale'].value = 2.0 - 0.75 * r2 * audioLevel || 0.3;
